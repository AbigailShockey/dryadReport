...
if (ar_tsv != "none") {
    sapply(strsplit(ar_tsv$Sample, "-"), "[[", 1)
    ar_genes <- c("blaOXA","blaKPC","blaVIM","blaIMP","blaNDM")
    ar_df <- dplyr::filter(ar_df, grepl(paste(ar_genes, collapse="|"), Gene))
    kable(ar_df) %>%
...
if (ar_mat != "none") {
  ...
  ar_mat <- ar_mata.frame(sapply(ar_mat[,], as.numeric))

  colnames(ar_mat) <- ar
  rownames(ar_mat) <- samples
  rownames(ar_mat) <- sapply(strsplit(rownames(ar_mat), "-"), "[[", 1)
...
scale_fill_viridis(limits = c(1,(max(snp.mat)+1)),
    na.value = "white",
    name = "SNPs",
    guide = "colourbar") +

...
The analysis of the *`r species`* samples you provided us (n = `r num.iso`) has been completed. This is what we were able to determine:
...
if (length(mpt$tip.label) > 10 & length(mpt$tip.label) < 16) {
  ymax <- max(gtree$data$y) + 1
  ymin <- min(gtree$data$y) - 9.5
  
  gheatmap(gtree, snp.mat, width = 20,
           cell_labels = TRUE,
           font.size = 2.5,
           cell_font_size = 2.25,
           offset = 40,
...
if (length(mpt$tip.label) < 11) {
  ...
  geom_nodelab(aes(label=label, x = branch, subset = !isTip & (as.numeric(label) > 95)), vjust=-.5, nudge_x = -.0005, size = 2) +
  # geom_label2(aes(label=label, x = branch, subset = !isTip & (as.numeric(label) > 95)), size = 2, label.size = .1, nudge_x = -.0002) +
  geom_tiplab(size = 2) +
  geom_treescale(offset = .1, y = 0, x = 0, fontsize = 2) +
...
if (length(mpt$tip.label) > 20 & length(mpt$tip.label) < 31) {
  ...
  # geom_label2(aes(label=label, x = branch, subset = !isTip & (as.numeric(label) > 95)), size = 2, label.size = .1, nudge_x = -.0002) +
  geom_tiplab(size = 2) +
  geom_treescale(offset = .2, y = 0, x = 0, fontsize = 2) +
  theme(plot.margin = unit(c(0,0,0,0), "cm"))
